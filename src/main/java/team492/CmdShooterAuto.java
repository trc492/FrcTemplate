...
import TrcCommonLib.trclib.TrcRobot.RobotCommand;
import TrcFrcLib.frclib.FrcRemoteVisionProcessor;

public class CmdShooterAuto implements RobotCommand
{
    ...
    public CmdShooterAuto(Robot robot)
    {
        ...
        if (robot != null)
        {
            robot.dashboard.putNumber("RobotVel", 0);
        }
    }
    ...
    public TrcPath createPickupPath(TrcPose2D start)
    {
        ...
        else if (startPosition == FrcAuto.StartPosition.IN_VISION)
        {
            ...
            Arrays.stream(path.getAllWaypoints()).filter(wp -> Math.abs(wp.pose.x - last.pose.x)<12).forEach(wp -> wp.velocity = 40);
            path.inferTimeSteps();
    ...
    private void localizeAtStart()
    {
        double x = startPosition != FrcAuto.StartPosition.CUSTOM ?
            ...
            robot.dashboard.getNumber(FrcAuto.CUSTOM_XPOS_KEY, 0);
    ...
    public boolean cmdPeriodic(double elapsedTime)
    {
        ...
        robot.dashboard.putNumber(
            "RobotVel", TrcUtil.magnitude(robot.driveBase.getXVelocity(), robot.driveBase.getYVelocity()));
        ...
        if (state != null)
        {
            ...
            switch (state)
            {
                ...
                case MOVE_TO_SHOOT:
                    ...
                    robot.purePursuit.setProximityRadius(12);
