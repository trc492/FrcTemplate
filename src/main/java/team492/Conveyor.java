...
public class Conveyor implements TrcExclusiveSubsystem
{
    ...
    public Conveyor(Robot robot)
    {
        ...
        advanceTask = TrcTaskMgr.createTask("ConveyorAdvanceTask", this::advanceTask);
        intakeTask = TrcTaskMgr.createTask("ConveyorIntakeTask", this::intakeTask);

        targetPosTicks = (int)motor.motor.getSelectedSensorPosition();
    }
    ...
    {
        if (!value && motor.getMotorPower() >= 0)
        {
            robot.decNumBalls();
    ...
    private void advanceTask(TrcTaskMgr.TaskType taskType, RunMode runMode)
    {
        if (onTarget())
        {
