...
public class Intake
{
    ...
    public Intake(Robot robot)
    {
        ...
        intakeExtension = new FrcPneumatic("IntakeExtension", RobotInfo.CANID_PCM, PneumaticsModuleType.CTREPCM, RobotInfo.SOL_INTAKE_EXTEND,
            RobotInfo.SOL_INTAKE_RETRACT);

        sm = new TrcStateMachine<>("Intake.sm");
        event = new TrcEvent("Intake.event");

        intakeTaskObj = TrcTaskMgr.createTask("IntakeTask", this::intakeTask);

        robot.dashboard.putNumber("IntakeState", -1);
        robot.dashboard.putNumber("EntranceSensor", 0);
    }
    ...
    private void intakeTask(TrcTaskMgr.TaskType taskType, RunMode runMode)
    {
        State state = sm.checkReadyAndGetState();
        robot.dashboard.putNumber("IntakeState", stateIndex());
        robot.dashboard.putNumber("EntranceSensor", robot.conveyor.entranceProximitySensor.isActive() ? 1 : 0);
        if (state != null)
        {
            robot.globalTracer
