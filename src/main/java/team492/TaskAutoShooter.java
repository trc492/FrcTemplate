...
public class TaskAutoShooter
{
    ...
    {
        this.robot = robot;
        taskObject = TrcTaskMgr.createTask(instanceName + ".shooterTask", this::shooterTask);
        ...
        robot.dashboard.putNumber("HeadingErr", 0);
    }
    ...
    private void shooterTask(TrcTaskMgr.TaskType taskType, RunMode runMode)
    {
...
//                double theta = Math.toDegrees(Math.atan2(RobotInfo.HIGH_TARGET_HEIGHT, depth));
        ...
        if (shouldAlign())
        {
            ...
            else
            {
                ...
                robot.dashboard.putNumber("HeadingErr", headingPid.getError());
        ...
        boolean lockInPlace)
    {
        ...
        isAligned = false;
        releaseDriveBase = !TrcOwnershipMgr.getInstance().hasOwnership(owner, robot.driveBase);
        releaseConveyor = !TrcOwnershipMgr.getInstance().hasOwnership(owner, robot.conveyor);
        if (shouldAlign() && !robot.driveBase.acquireExclusiveAccess(owner))
        {
            ...
            robot.globalTracer
                .traceErr(owner + ".shoot", "Unable to acquire exclusive access of drivebase! CurrOwner=%s",
                    TrcOwnershipMgr.getInstance().getOwner(robot.driveBase));
            return;
        }
        if (shouldShoot() && !robot.conveyor.acquireExclusiveAccess(owner))
        {
            ...
            robot.globalTracer
                .traceErr(owner + ".shoot", "Unable to acquire exclusive access of conveyor! CurrOwner=%s",
                    TrcOwnershipMgr.getInstance().getOwner(robot.conveyor));
            return;
        }
        this.owner = owner;
